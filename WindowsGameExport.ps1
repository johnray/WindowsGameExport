...
function Get-BatContentForComparison {
    # Generate .bat content without timestamp for comparison purposes
...
function Update-GameLaunchers {
    ...
    foreach ($game in $Games) {
        ...
        # Compare content directly (no state file needed)
        $contentMatches = $false
        if ($fileExists) {
            $expectedContent = Get-BatContentForComparison -Game $game
            $existingContent = Get-Content -Path $filePath -Raw -ErrorAction SilentlyContinue
            # Remove timestamp line from existing content for comparison
            $existingForComparison = ($existingContent -split "`r?`n" | Where-Object { $_ -notmatch '^REM Generated:' }) -join "`n"
            $expectedForComparison = ($expectedContent -split "`r?`n") -join "`n"
            # Normalize non-ASCII characters for comparison (they may get mangled by encoding)
            $existingNormalized = $existingForComparison -replace '[^\x00-\x7F]+', '?' -replace '\?+', '?'
            $expectedNormalized = $expectedForComparison -replace '[^\x00-\x7F]+', '?' -replace '\?+', '?'
            $contentMatches = $existingNormalized.Trim() -eq $expectedNormalized.Trim()
        }
    ...
    # Remove orphaned .bat files (only those generated by this utility)
    if ($OutputDirectory -and -not $DryRun) {
        ...
        foreach ($file in $existingFiles) {
            if (-not $expectedFiles.ContainsKey($file.FullName)) {
                # Only delete if file has our signature (was generated by this utility)
                $content = Get-Content -Path $file.FullName -TotalCount 3 -ErrorAction SilentlyContinue
                $hasSignature = $content -and ($content -join "`n") -match 'REM \[WindowsGameExport\]'
                if ($hasSignature) {
...
Write-Host "  ------------------------------------------------------------" -ForegroundColor DarkGray
$results = Update-GameLaunchers -Games $allGames -OutputDirectory $OutputDirectory -DryRun:$DryRun -IgnoreUnverified:$IgnoreUnverified
